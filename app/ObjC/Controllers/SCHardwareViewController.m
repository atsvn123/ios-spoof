...
- (UITableViewCell*)tableView:(UITableView*)t cellForRowAtIndexPath:(NSIndexPath*)i{ NSDictionary*p=[self.config resolvedPreset]; if(i.section==0){NSArray*k=@[@"capacityGB",@"chipId",@"cpuArchitecture",@"deviceClass",@"boardId"];return[self cellWithTitle:@[@"Storage",@"Chip",@"CPU",@"Class",@"Board"][i.row] detail:[p[k[i.row]] description]];} NSArray *titles=@[@"Spoof Battery",@"Hide Jailbreak",@"Random IDs mỗi launch"]; NSArray *values=@[@(self.config.spoofBattery),@(self.config.hideJailbreak),@(self.config.randomizeOnLaunch)]; return [self switchCellWithTitle:titles[i.row] on:[values[i.row] boolValue] action:@selector(toggle:)];}

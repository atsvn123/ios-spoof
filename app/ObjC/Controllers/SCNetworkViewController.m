...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)i {
    ...
    if (i.section == 1) {
        NSArray *titles = @[@"Ẩn Proxy", @"Ẩn VPN", @"Ẩn Jailbreak"];
        NSArray *values = @[@(self.config.hideProxy), @(self.config.hideVPN), @(self.config.hideJailbreak)];
        return [self switchCellWithTitle:titles[i.row] on:[values[i.row] boolValue] action:@selector(toggleAnti:)];
    }
    NSArray *titles = @[@"Spoof IDFA", @"Spoof IDFV", @"Spoof Battery"];
    NSArray *values = @[@(self.config.spoofIDFA), @(self.config.spoofIDFV), @(self.config.spoofBattery)];
    return [self switchCellWithTitle:titles[i.row] on:[values[i.row] boolValue] action:@selector(toggleIDs:)];
}

...
static NSString *SCEffectiveTimezoneIdentifier(void) {
    if (CFG().timezoneIdentifier.length) {
        if ([CFG().timezoneIdentifier isEqualToString:@"Asia/Saigon"]) return @"Asia/Ho_Chi_Minh";
        return CFG().timezoneIdentifier;
    }
...
static NSString *SCWebKitSpoofScript(void) {
    ...
    double alt = CFG().altitude;
    double heading = CFG().heading;
    NSInteger tzOffsetMinutes = [[NSTimeZone timeZoneWithName:tz] secondsFromGMT] / 60;
    NSString *seedString = [NSString stringWithFormat:@"%@|%@|%@|%@", CFG().spoofedUDID ?: @"", P().productType ?: @"", CFG().buildID ?: @"", CFG().pasteboardUUID ?: @""];
    uint32_t seed = 2166136261u;
    NSData *seedData = [seedString dataUsingEncoding:NSUTF8StringEncoding] ?: [NSData data];
    const uint8_t *bytes = seedData.bytes;
    for (NSUInteger idx = 0; idx < seedData.length; idx++) {
    ...
    return [NSString stringWithFormat:
        @"(()=>{"
         ...
         "const seed=%u>>>0;const hash=(s)=>{let h=seed;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619);}return h>>>0;};const unit=(s)=>((hash(s)%%10000)/10000);const offset=(s,m)=>((unit(s)*2)-1)*m;"
         ...
         "const patchGL=(proto)=>{if(!proto)return;const gp=proto.getParameter;proto.getParameter=function(p){if(p===37445)return '%@';if(p===37446)return '%@';return gp.call(this,p);};maskNative(proto.getParameter,'getParameter');};patchGL(window.WebGLRenderingContext&&WebGLRenderingContext.prototype);patchGL(window.WebGL2RenderingContext&&WebGL2RenderingContext.prototype);"
          "const scrubSDP=(s)=>String(s||'').replace(/a=candidate:.*\\r?\\n/g,'').replace(/c=IN IP4 .*\\r?\\n/g,'c=IN IP4 0.0.0.0\\r\\n').replace(/c=IN IP6 .*\\r?\\n/g,'c=IN IP6 ::\\r\\n');"
          "const patchRTCProto=(RTC)=>{if(!RTC||!RTC.prototype||RTC.prototype.__iosspoof_rtc)return;try{Object.defineProperty(RTC.prototype,'__iosspoof_rtc',{value:1});}catch(e){}const o=RTC.prototype.createOffer;if(o){RTC.prototype.createOffer=function(...args){return o.apply(this,args).then(d=>({type:d.type,sdp:scrubSDP(d.sdp)}));};maskNative(RTC.prototype.createOffer,'createOffer');}const a=RTC.prototype.createAnswer;if(a){RTC.prototype.createAnswer=function(...args){return a.apply(this,args).then(d=>({type:d.type,sdp:scrubSDP(d.sdp)}));};maskNative(RTC.prototype.createAnswer,'createAnswer');}const sld=RTC.prototype.setLocalDescription;if(sld){RTC.prototype.setLocalDescription=function(desc){if(desc&&desc.sdp)desc={type:desc.type,sdp:scrubSDP(desc.sdp)};return sld.call(this,desc);};maskNative(RTC.prototype.setLocalDescription,'setLocalDescription');}const sc=RTC.prototype.setConfiguration;if(sc){RTC.prototype.setConfiguration=function(cfg){try{cfg=Object.assign({},cfg||{});cfg.iceServers=[];}catch(e){}return sc.call(this,cfg);};maskNative(RTC.prototype.setConfiguration,'setConfiguration');}const gc=RTC.prototype.getConfiguration;if(gc){RTC.prototype.getConfiguration=function(){const cfg=gc.call(this)||{};try{cfg.iceServers=[];}catch(e){}return cfg;};maskNative(RTC.prototype.getConfiguration,'getConfiguration');}['localDescription','currentLocalDescription','pendingLocalDescription'].forEach(k=>{try{const od=Object.getOwnPropertyDescriptor(RTC.prototype,k);if(od&&od.get)Object.defineProperty(RTC.prototype,k,{get:function(){const d=od.get.call(this);return d&&d.sdp?{type:d.type,sdp:scrubSDP(d.sdp)}:d;},configurable:true});}catch(e){}});const ald=RTC.prototype.addEventListener;if(ald){RTC.prototype.addEventListener=function(t,l,o){if(t==='icecandidate'&&l){const w=function(ev){try{if(ev&&ev.candidate)return;}catch(e){}return (typeof l==='function'?l.call(this,ev):l.handleEvent&&l.handleEvent(ev));};return ald.call(this,t,w,o);}return ald.call(this,t,l,o);};maskNative(RTC.prototype.addEventListener,'addEventListener');}try{Object.defineProperty(RTC.prototype,'onicecandidate',{get(){return this.__iosspoof_onicecandidate||null;},set(fn){this.__iosspoof_onicecandidate=fn&&function(ev){try{if(ev&&ev.candidate)return;}catch(e){}return fn.call(this,ev);};},configurable:true});}catch(e){}const gcs=RTC.prototype.getStats;if(gcs){RTC.prototype.getStats=function(...args){return gcs.apply(this,args).then(r=>{try{r.forEach(v=>{if(v&&(v.type==='local-candidate'||v.type==='remote-candidate'||v.candidateType)){v.ip='0.0.0.0';v.address='0.0.0.0';v.port=0;v.url='';v.candidate='';}});}catch(e){}return r;});};maskNative(RTC.prototype.getStats,'getStats');}};"
          "patchRTCProto(window.RTCPeerConnection);patchRTCProto(window.webkitRTCPeerConnection);"
          ...
          "})();",
         seed, (long)width, (long)height, (long)width, (long)height, scale, tz, locale, (long)-tzOffsetMinutes, lat, lon, acc, alt, acc, heading, gpuVendor, gpuRenderer];
}

...
- (CGRect)bounds {
    if (SC_ON() && P()) {
        return CGRectMake(0, 0, P().screenWidth, P().screenHeight);
    }
    return %orig;
}
- (CGFloat)scale {
...
// ============================================================================
//  3. NSBundle - ProductType / hw.machine / identifiers
// ============================================================================
%hook NSBundle
...
- (NSString *)objectForInfoDictionaryKey:(NSString *)key {
    if (SC_ON() && P()) {
        if ([key isEqualToString:@"CFBundleIdentifier"]) {
            // giữ nguyên bundle id thật
        } else if ([key isEqualToString:@"DTPlatformName"] ||
                   [key isEqualToString:@"DTSDKName"] ||
                   [key isEqualToString:@"MinimumOSVersion"]) {
            // giữ native
        }
    }
    return %orig;
...
- (id)objectForInfoDictionaryKey:(NSString *)key {  // overloaded
    return %orig;
}
...
// ============================================================================
//  4. sysctlbyname / sysctl - hw.machine, hw.model, hw.serialnumber, hw.UUID
...
extern io_registry_entry_t IORegistryEntryFromPath(CFAllocatorRef, const char *);
extern kern_return_t IOObjectRelease(io_object_t);
extern CFStringRef IORegistryEntryGetName(io_registry_entry_t, char *);
...
static CFTypeRef sc_IORegistryEntryCreateCFProperty(io_registry_entry_t entry,
    ...
    if (SC_ON() && P()) {
        ...
        if ([k isEqualToString:@"IOPlatformECID"]) {
            return (__bridge_retained CFTypeRef)CFG().spoofedECID;
        }
        if ([k isEqualToString:@"board-id"]) {
            return (__bridge_retained CFTypeRef)P().boardId;
        }
        if ([k isEqualToString:@"product-name"]) {
            return (__bridge_retained CFTypeRef)P().productName;
        }
        if ([k isEqualToString:@"model"]) {
            return (__bridge_retained CFTypeRef)P().hardwareModel;
        }
...
static pid_t sc_fork(void) {
    if (SC_ON() && CFG().hideJailbreak) {
        // một số app gọi fork để check; trả về -1 (không hỗ trợ)
    }
    return orig_fork();
}
...
// ============================================================================
//  8. Battery - UIDevice battery level/state
// ============================================================================
%hook UIDevice
- (float)batteryLevel {
...
// ============================================================================
//  9. UIScreen native (re-hook vì đã có group trên)
//    Đã cover ở section 2.
// ============================================================================

// ============================================================================
//  10. CFBundleGetValueForInfoDictionaryKey - marketing name, device info
// ============================================================================
...
// ============================================================================
//  11. sysctl CTL_HW / CTL_KERN chung (uname)
// ============================================================================
